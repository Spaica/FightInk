...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        if let component = player.component(ofType: SpriteComponent.self) {
            ...
            self.spriteComponent?.node.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
...
//
    override func keyDown(with event: NSEvent) {
        ...
        case 0xD:
            let currentPosition = self.spriteComponent?.node.position
            self.spriteComponent?.node.position = CGPoint(x: currentPosition!.x, y: currentPosition!.y + 5)
        default:
            print("ciao")
        }
    }
